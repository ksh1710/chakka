...
# Initialize Firebase
firestore_client, database_ref = init_firebase()
...
def fetch_user_data():
    try:
        user_data = database_ref.child('UsersData').child("wmKpLeRocgRwhVf10Fe3cTlSlnw2").get()
        return user_data
    except Exception as e:
        print(f"Error fetching user data for ID wmKpLeRocgRwhVf10Fe3cTlSlnw2: {e}")
        return None
...
def init_routes(app):
    ...
    def fetch_all_users():
        ...
        return jsonify({'status': 'error', 'message': 'Failed to fetch users data'})
